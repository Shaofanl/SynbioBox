...
static function changeDropSpeed(alpha: float) {	
	drop_speed *= alpha;
...
/*	if (isStageMode) {
	...
	if (isEndlessMode) {	
		...
		if (c == 'A') main.sprite = tips[3].GetComponent.<SpriteRenderer>().sprite; // 0
		if (c == 'B') main.sprite = tips[0].GetComponent.<SpriteRenderer>().sprite; // 1
		if (c == 'C') main.sprite = tips[2].GetComponent.<SpriteRenderer>().sprite; // 2
		if (c == 'D') main.sprite = tips[1].GetComponent.<SpriteRenderer>().sprite; // 3
	}
}
	...
	if (isEndlessMode) {
		...
		if (isGodMode) {
			...
			if (godtime > 10) {
				...
				MusicControl.ChangePitch(1.0);
				changeDropSpeed(default_drop_speed*Mathf.Log(speed_level+1)/drop_speed);
			}
			else {
...
static function energyUp(delta : int) {
	...
	if (energy_bar >= 100) {
		...
		changeDropSpeed(default_drop_speed*Mathf.Log(speed_level+2)/drop_speed);
